...
public class IntakeHelper {
    ...
    public final static IntakeHelper INSTANCE = new IntakeHelper();
    ...
    private Launchers launcherHelper = new Launchers();
    ...
    public void update() {
        ...
        if (!jammed) {
            if (getFirstEmptyLauncher() != null && !hasBall) {
                leftDistance = DistanceSensors.getDistance(DistanceSensors.Left());
                rightDistance = DistanceSensors.getDistance(DistanceSensors.Right());
        ...
        intake(); // Run the unified state machine
    }
