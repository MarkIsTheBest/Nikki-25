...
import static subsystems.Helper.SetMotorPosition;
...
public final class ServoMotorConfigTest extends LinearOpMode {
    ...
    // Servo configuration
    private final Servo[] servosCH = new Servo[SERVO_PIN_COUNT]; // Servos on Control Hub
    private final Servo[] servosEH = new Servo[SERVO_PIN_COUNT]; // Servos on Expansion Hub
    ...
    private void update() {
        ...
        if (apply) {
            ...
            switch (motorID) {
                // Drive motors
                case 0:
                    SetMotorPosition(Motors.leftFront, M_Drive._0_LeftFrontPosition);
                    break;
                case 1:
                    SetMotorPosition(Motors.rightFront, M_Drive._1_RightFrontPosition);
                    break;
                case 2:
                    SetMotorPosition(Motors.leftRear, M_Drive._2_LeftBackPosition);
                    break;
                case 3:
                    SetMotorPosition(Motors.rightRear, M_Drive._3_RightBackPosition);
                    break;
                // Slider motors
                case 4:
